...
export function FretPrimer() {
  return (
    <div>
      ...
      <ul className="mb-3 divide-y divide-gray-100 rounded-lg border border-[var(--panel-line)] dark:divide-gray-800">
        {STRINGS.map((s) => (
          <li key={s.no} className="flex items-center gap-2 px-2.5 py-1.5">
            ...
            <span className="min-w-0 flex-1 text-[11px] text-[color-mix(in_srgb,var(--foreground)_55%,transparent)]">{s.note}</span>
          </li>
        ))}
      </ul>
      ...
      <div className="mb-3 grid grid-cols-7 gap-1 text-center">
        {SOLFEGE.map(([ko, en]) => (
          <div
            key={en}
            className="rounded border border-[var(--panel-line)] py-1.5"
          >
            <div className="text-[11px] text-[color-mix(in_srgb,var(--foreground)_55%,transparent)]">{ko}</div>
      ...
      <Fretboard />
      <p className="mt-2 text-[11px] leading-relaxed text-[color-mix(in_srgb,var(--foreground)_55%,transparent)]">
