...
| 설정 | 분석 서버 주소, 음원 분리, 파형 확대, 코드 표기(♯/♭), 서버 상태 |

### 분석 서버 주소 지정
설정 탭에서 서버 주소를 직접 넣고 「연결 테스트」로 확인할 수 있다.
비워 두면 페이지와 같은 주소를 쓴다(집 안에서 쓰는 방식, 프록시가 처리).

주소 결정 우선순위는 **설정 > 빌드 시 `NEXT_PUBLIC_API_BASE` > 같은 주소**.

> **https 페이지에서 http 서버는 호출할 수 없다.** 브라우저가 막는다.
> GitHub Pages(https)에 올린 화면으로 집 서버를 쓰려면 서버도 https여야 한다.
> 설정 화면이 이 상황을 감지해 경고를 띄운다.
