# YouTube → 기타 코드 자동 생성 웹앱 (Chord AI 클론) 개발 계획
...
- [ ] **M1** — ffmpeg 디코드 + yt-dlp 실제 다운로드 경로 (`pipeline.py`의 `TODO(M1)`)
- [ ] **M2** — 비트/다운비트 + 템플릿 코드 인식
- [ ] **M3** — YouTube 플레이어 동기화 타임라인 + 프렛보드 다이어그램
- [ ] **M4** — Demucs + 사전학습 코드 모델 (GPU)
- [ ] **M5** — Chord AI 대조 평가
- [ ] **M6** — 이조/카포/루프/수동편집
- [ ] **M7** — 캐시·에러처리·모바일 마무리
